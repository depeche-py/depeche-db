...
class FooRepoWithCache(EventStoreRepo[FooEvent, Foo, _uuid.UUID]):
    def __init__(self, event_store: MessageStore[FooEvent], cache=None):
        super().__init__(
...
def test_inmemory_cache_get_returns_none_for_missing():
    cache = InMemoryCache()
    assert cache.get(_uuid.uuid4()) is None


def test_inmemory_cache_set_and_get():
    cache = InMemoryCache(cache_every_x_version=1)
    foo = Foo.create(_uuid.uuid4(), "test")
    cache.set(foo.id, foo)
...
def test_inmemory_cache_returns_copy():
    cache = InMemoryCache(cache_every_x_version=1)
    foo = Foo.create(_uuid.uuid4(), "test")
    cache.set(foo.id, foo)
...
def test_inmemory_cache_every_x_versions():
    cache = InMemoryCache(cache_every_x_version=5)
    foo = Foo.create(_uuid.uuid4(), "test")
...
def test_repo_without_cache(identifier, db_engine):
...
def test_repo_with_cache(identifier, db_engine):
    store = MessageStore[FooEvent](
        name=identifier(),
        engine=db_engine,
        serializer=PydanticMessageSerializer(FooEvent),
...
def test_cache_is_updated_with_new_events(identifier, db_engine):
    store = MessageStore[FooEvent](
        name=identifier(),
        engine=db_engine,
        serializer=PydanticMessageSerializer(FooEvent),
...
def test_cache_hit_replays_only_new_events(identifier, db_engine):
    store = MessageStore[FooEvent](
        name=identifier(),
        engine=db_engine,
        serializer=PydanticMessageSerializer(FooEvent),
